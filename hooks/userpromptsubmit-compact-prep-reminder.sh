...
#
# 200K context 環境向け調整:
#   閾値〜自動compact発火点(90〜95%)までの余白が数十Kトークンしかなく、
#   区切りを待つ猶予がないため「即座に」実行を促す文言にしている。
#
# フロー:
...
CTX="[COMPACT PREP REMINDER] context 使用率が ${CTX_PCT}% に達した。自動 compact 発火点までの余白が少ないため即座に対応せよ。"
CTX+=$'\n'"- 区切りを待たず、今すぐユーザーに \`/compact-prep\` の実行を提案せよ。"
